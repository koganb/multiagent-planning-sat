...
@Unroll
class TestDelayFailureModelWithRetriesRaw extends Specification {

    private static final Logger log

    static {
        System.properties.'TEST_NAME' = 'DelayFailureModel_WithRetries_Raw'
        log = LoggerFactory.getLogger(TestDelayFailureModelWithRetriesRaw.class)
    }
    ...
    def "test diagnostics calculation for plan: #problemName, failures: #failedActions "(
        ...
        [problemName, plan, cnfPlanClauses, failedActions] <<
                [[problemArr, planArr, cnfPlanClausesArr].transpose(), [1, 2, 3]]
                        ...
                        .collect {
                    [it, PreparedTestActionReader.getTestActions(it[0], String.format("testCases/raw/%s.problem_%s.yml", it[0], it[3])), "DelayFailureModelWithRetriesParams"]
                }
                .collect {
