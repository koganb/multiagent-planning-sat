...
@Unroll
class TestNoEffectsFailureModelWithRetriesRaw extends Specification {

    private static final Logger log

    static {
        System.properties.'TEST_NAME' = 'NoEffectsFailureModel_WithRetries_Raw'
        log = LoggerFactory.getLogger(TestNoEffectsFailureModelWithRetriesRaw.class)
    }
...
//            ["2-4", "2-5", "2-6", "3-6", "3-7", "3-8", "3-10", "5-10", "5-15", "5-20", "5-25", "5-26"].collect {"ZTRAVEL-${it}"}
    ...
    def "test diagnostics calculation for plan: #problemName, failures: #failedActions "(
        ...
        [problemName, plan, cnfPlanClauses, failedActions] <<
                [[problemArr, planArr, cnfPlanClausesArr].transpose(), [1, 2, 3]]
                        .combinations()
                        .collect { [it[0][0], it[0][1], it[0][2], it[1]] }
                        .collect {
                    [it, PreparedTestActionReader.getTestActions(it[0], String.format("testCases/raw/%s.problem_%s.yml", it[0], it[3])), "NoEffectsFailureModelWithRetriesParams"]
                }
                .collect {
